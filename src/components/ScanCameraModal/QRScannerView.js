...
class QRScannerRectView extends Component {
  ...
  renderLoadingIndicator() {
    ...
    return <ActivityIndicator animating={this.props.isLoading} color={this.props.color} size="large" />;
  }
  ...
  _renderScanBar() {
    ...
    if (this.props.scanBarImage) {
      return <Image style={{ resizeMode: "contain", width: this.getScanImageWidth() }} source={this.props.scanBarImage} />;
    } else {
      return (
        <View
          style={[
  ...
  render() {
    ...
    return (
      <View onLayout={({ nativeEvent: e }) => this.measureTotalSize(e)} style={[styles.container, this.getBottomMenuHeight()]}>
        <View style={[styles.viewfinder, this.getRectSize()]} onLayout={({ nativeEvent: e }) => this.measureRectPosition(e)}>
          {/*扫描框边线*/}
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        >
          ...
          {/*绘制底部操作栏*/}
          <View style={[styles.buttonsContainer, this.props.bottomMenuStyle]}>{this.props.renderBottomMenuView()}</View>
        </RNCamera>
      </View>
    );
  }
